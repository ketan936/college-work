...
{	
	...
	while(1)
	{
		...
		switch(choice)
		{
			case 1:
			{
				string allfile,encrypted,key,key2;
				printf("Enter key : ");
				cin>>key;
				...
				{
				    encrypted.push_back(((allfile[i]-'a')+(key2[key[i%length_key]-'a']-'a'))%26 + 'a');
				}
			...
			case 2:
			{
				...
				for(i=0;i<allfile.length();i++)
				{
				    plain.push_back(((allfile[i]-'a')-(key2[key[i%length_key]-'a']-'a') + 26)%26 + 'a');
				}
