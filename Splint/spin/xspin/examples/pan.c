...
#ifdef VAR_RANGES
		logval("node:Active", ((P0 *)pptr(h))->Active);
		logval("node:know_winner", ((P0 *)pptr(h))->know_winner);
		logval("node:mynumber", ((P0 *)pptr(h))->mynumber);
		logval("node:nr", ((P0 *)pptr(h))->nr);
		logval("node:maximum", ((P0 *)pptr(h))->maximum);
		logval("node:neighbourR", ((P0 *)pptr(h))->neighbourR);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
#ifndef XUSAFE
		setq_claim(((P0 *)this)->out, 2, "out", h, "node");
		setq_claim(((P0 *)this)->in, 1, "in", h, "node");
#endif
		break;
	}
	this = o_this;
...
{
		now.nr_leaders = 0;
		now.q[0] = addqueue(1, 0);
		now.q[1] = addqueue(2, 0);
		now.q[2] = addqueue(3, 0);
		now.q[3] = addqueue(4, 0);
		now.q[4] = addqueue(5, 0);
#ifdef VAR_RANGES
		logval("nr_leaders", now.nr_leaders);
#endif
	Maxbody = max(Maxbody, sizeof(Q1));
	Maxbody = max(Maxbody, sizeof(Q2));
	Maxbody = max(Maxbody, sizeof(Q3));
	Maxbody = max(Maxbody, sizeof(Q4));
	Maxbody = max(Maxbody, sizeof(Q5));
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full(now.q[0])
			 ||     q_full(now.q[1])
			 ||     q_full(now.q[2])
			 ||     q_full(now.q[3])
			 ||     q_full(now.q[4])))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(now.q[0])
			 ||    !q_full(now.q[1])
			 ||    !q_full(now.q[2])
			 ||    !q_full(now.q[3])
			 ||    !q_full(now.q[4])))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(now.q[0])
			 ||     !q_len(now.q[1])
			 ||     !q_len(now.q[2])
			 ||     !q_len(now.q[3])
			 ||     !q_len(now.q[4])))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(now.q[0])
			 ||      q_len(now.q[1])
			 ||      q_len(now.q[2])
			 ||      q_len(now.q[3])
			 ||      q_len(now.q[4])))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P0 *)this)->in)))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full(((P0 *)this)->in)))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len(((P0 *)this)->in)))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len(((P0 *)this)->in)))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full(((P0 *)this)->out)))
