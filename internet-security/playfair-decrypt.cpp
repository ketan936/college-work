...
while(k<message.length())
{
    ...
    if(r1==r2)
    {
        encrypted.push_back(look[r1][(c1-1+row)%row]);
        encrypted.push_back(look[r1][(c2-1+row)%row]);
    }
    else if(c1==c2)
    {
        encrypted.push_back(look[(r1-1+column)%column][c1]);
        encrypted.push_back(look[(r2-1+column)%column][c2]);
    }
    else
    {
